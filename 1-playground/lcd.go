...
var (
	ENABLE_DELAY               = 10 * time.Microsecond
	EXECUTION_TIME_DEFAULT     = 400 * time.Microsecond
	EXECUTION_TIME_RETURN_HOME = 15200 * time.Microsecond
)
...
// global used to ensure the rpio library is initialized before using it.
...
func NewLcd1602(rs, e gpio.PinIO, data []gpio.PinIO, linewidth int) *LCD {
	...
	datapins := make([]gpio.PinIO, 0)

	for _, d := range data {
		datapins = append(datapins, d)
	}
...
func setBitToPin(pin gpio.PinIO, data, position uint8) {
